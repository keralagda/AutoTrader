...
async function main() {
  console.log('--- Starting Final Binary Tree Audit Logging Test ---')

  try {
    // Clean up any old test data
    await db.binaryTreeAuditLog.deleteMany({})
    await db.user.deleteMany({ where: { email: { startsWith: 'test_final_' } } })
    await db.plan.deleteMany({ where: { name: { startsWith: 'TEST_FINAL_' } } })

    // 1. Create a test plan with binary MLM enabled
    const plan = await db.plan.create({
      data: {
        name: 'TEST_FINAL_PLAN_BINARY',
        entryFee: 10,
        minDeposit: 100,
        maxDeposit: 10000,
    ...
    // 2. Create test users (Sponsor + Child) - using bcrypt directly
    const passwordHash = await bcrypt.hash('password123', 12)
    const sponsor = await db.user.create({
      data: {
        email: 'test_final_sponsor@test.com',
        name: 'Final Sponsor',
        password: passwordHash,
        referralCode: 'FINALSP01',
        isActive: true,
      }
    })
    ...
    const child = await db.user.create({
      data: {
        email: 'test_final_child@test.com',
        name: 'Final Child',
        password: passwordHash,
        referralCode: 'FINALCH01',
    ...
    // Optional: Check if we can see the details in the logs
    if (placementLog.actionDetails) {
      console.log('✓ Placement log has details:', placementLog.actionDetails)
    }
    if (volumeLog.actionDetails) {
      console.log('✓ Volume update log has details:', volumeLog.actionDetails)
    }

    // Clean up test data
    await db.earning.deleteMany({ where: { user: { email: { startsWith: 'test_final_' } } } })
    await db.transactionLog.deleteMany({ where: { user: { email: { startsWith: 'test_final_' } } } })
    await db.notification.deleteMany({ where: { user: { email: { startsWith: 'test_final_' } } } })
    await db.deposit.deleteMany({ where: { user: { email: { startsWith: 'test_final_' } } } })
    await db.user.deleteMany({ where: { email: { startsWith: 'test_final_' } } })
    await db.plan.deleteMany({ where: { name: { startsWith: 'TEST_FINAL_' } } })
    await db.binaryTreeAuditLog.deleteMany({ where: { OR: [{ userId: sponsor.id }, { userId: child.id }] } })

    console.log('✓ Cleaned up test data.')
    console.log('--- FINAL AUDIT LOGGING TEST PASSED! ---')
    return true;
  } catch (error) {
