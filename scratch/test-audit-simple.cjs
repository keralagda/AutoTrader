...
const { placeUserInBinaryTree } = require('../src/lib/binary-tree');
const { updateBinaryTreeVolumes } = require('../src/lib/binary-tree');

const db = new PrismaClient();

async function main() {
  console.log('--- Starting Simple Binary Tree Audit Logging Test ---');

  try {
    // Clean up any old test data
    await db.binaryTreeAuditLog.deleteMany({});
    await db.user.deleteMany({ where: { email: { startsWith: 'test_simple_' } } });
    await db.plan.deleteMany({ where: { name: { startsWith: 'TEST_SIMPLE_' } } });

    // 1. Create a test plan with binary MLM enabled
    const plan = await db.plan.create({
      data: {
        name: 'TEST_SIMPLE_PLAN_BINARY',
        entryFee: 10,
        minDeposit: 100,
        maxDeposit: 10000,
    ...
    const sponsor = await db.user.create({
      data: {
        email: 'test_simple_sponsor@test.com',
        name: 'Simple Sponsor',
        password,
        referralCode: 'SIMplesp01',
        isActive: true,
      }
    });
    ...
    const child = await db.user.create({
      data: {
        email: 'test_simple_child@test.com',
        name: 'Simple Child',
        password,
        referralCode: 'SIMplech01',
    ...
    // Clean up test data
    await db.earning.deleteMany({ where: { user: { email: { startsWith: 'test_simple_' } } } });
    await db.transactionLog.deleteMany({ where: { user: { email: { startsWith: 'test_simple_' } } } });
    await db.notification.deleteMany({ where: { user: { email: { startsWith: 'test_simple_' } } } });
    await db.deposit.deleteMany({ where: { user: { email: { startsWith: 'test_simple_' } } } });
    await db.user.deleteMany({ where: { email: { startsWith: 'test_simple_' } } });
    await db.plan.deleteMany({ where: { name: { startsWith: 'TEST_SIMPLE_' } } });
    await db.binaryTreeAuditLog.deleteMany({ where: { OR: [{ userId: sponsor.id }, { userId: child.id }] } });

    console.log('✓ Cleaned up test data.');
    console.log('--- SIMPLE AUDIT LOGGING TEST PASSED! ---');
    return true;
  } catch (error) {
    ...
    try {
      ...
      await db.user.deleteMany({ where: { email: { startsWith: 'test_simple_' } } });
      await db.plan.deleteMany({ where: { name: { startsWith: 'TEST_SIMPLE_' } } });
      await db.binaryTreeAuditLog.deleteMany({ where: { OR: [{ userId: sponsorId ? sponsorId : undefined, userId: childId ? childId : undefined }] } });
    } catch (cleanupError) {
