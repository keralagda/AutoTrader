...
const LOGO_URL = '/logo-bnfx-gold.png' // Will use the uploaded logo
        ...
        <TabsContent value="free" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredResources.map(resource => (
              <Card key={resource.id} className="border-border/50 hover:border-amber-500/30 transition-colors group">
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="size-12 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-2xl">
                      {resource.preview}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="h-7 text-xs gap-1 group-hover:bg-amber-500/10 group-hover:border-amber-500/30 group-hover:text-amber-400"
            ...
            {PAID_MERCH.map(item => {
              ...
              return (
                <Card key={item.id} className={`border-border/50 transition-all ${item.inStock ? 'hover:border-amber-500/30' : 'opacity-60'}`}>
                  <CardContent className="p-4 space-y-3">
                    {/* Product Image */}
                    <div className="relative h-32 rounded-xl bg-gradient-to-br from-amber-500/5 to-card border border-border/30 flex items-center justify-center">
                      <span className="text-5xl">{item.image}</span>
                      {!item.inStock && (
                        <div className="absolute inset-0 bg-background/60 rounded-xl flex items-center justify-center">
                          <Badge className="bg-rose-500/20 text-rose-400 border-rose-500/30">Out of Stock</Badge>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-lg font-bold text-amber-400" dir="ltr">${item.price}</span>
                      </div>
